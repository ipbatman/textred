...
const DICT_STOP = ['вот', 'сейчас', 'от',
  ...
  'они', 'из', 'ему', 'теперь', 'когда', 'даже', 'ну', 'вдруг', 'ли',
  'если', 'уже', 'или', 'ни', 'быть', 'был', 'него', 'до', 'вас',
  'нибудь', 'опять', 'уж', 'вам', 'ведь', 'там', 'потом', 'себя',
  'ничего', 'ей', 'может', 'они', 'над', 'больше', 'очень', 'через',
  'день', 'этот', 'тот', 'эти', 'те', 'мой', 'твой', 'наш', 'ваш'
];
const DICT_CLICHE = ['в настоящее время', 'на сегодняшний день', 'на данный момент', 'следует отметить',
  'стоит отметить', 'как уже было сказано', 'в связи с этим', 'на самом деле', 'безусловно', 'без сомнения',
  'в общем и целом', 'по большому счету', 'так или иначе', 'тем не менее', 'в первую очередь', 'как таковой',
  'имеет место', 'в рамках', 'в целях', 'в сфере', 'в области', 'является', 'осуществляет', 'производит'];
...
const DICT_AMPLIFIER = ['Самые', 'особенно', 'самый', 'точно', 'фактическая', 'очень',
  'крайне', 'чрезвычайно', 'абсолютно', 'совершенно', 'действительно', 'безусловно', 'несомненно',
  'прямо', 'просто', 'даже', 'уж', 'чертовски', 'невероятно', 'максимально', 'предельно', 'безумно',
  'жутко', 'страшно', 'капитально', 'реально', 'по-настоящему', 'в высшей степени', 'сверх', 'исключительно'
];
...
const DICT_STAMP = ['То же самое', 'повестки дня', 'в политической жизни', 'лучший на рынке',
  ...
  'гарантированный результат', 'эксклюзивные условия', 'революционный прорыв', 'самые низкие цены',
  'проверенный временем', 'абсолютная безопасность', '100% результат', 'мировой уровень',
  'первоклассный сервис', 'высококвалифицированные специалисты'
];
const DICT_WEAK = ['был', 'Есть', 'является', 'существовало', 'существуют', 'являются', 'являться',
  'осуществляет', 'осуществляют', 'осуществлять', 'производит', 'производят', 'производить',
  'процесс', 'осуществление', 'производство', 'возможность', 'наличии', 'отсутствии', 'случае',
  'связи', 'причине', 'факт', 'вопрос', 'уровне', 'целях', 'рамках', 'порядке', 'основе'
];
const DICT_VAGUE = ['многие', 'достаточно', 'вроде', 'вроде бы', 'кажется', 'наверное', 'возможно',
  'может быть', 'примерно', 'приблизительно', 'где-то', 'около',  'какой-то', 'некий', 'некоторые',
  'определенный', 'чуть ли не', 'вряд ли', 'по всей видимости', 'скорее всего', 'в каком-то смысле',
  'условно', 'предположительно', 'как бы', 'чего-либо'
];
const DIST_PERSONAL = ['его', 'оно', 'ему', 'они', 'Их', 'мы', 'ее', 'он',
  'я', 'меня', 'мне', 'мной', 'ты', 'тебя', 'тебе', 'тобой', 'его', 'нему', 'ним', 'она', 'её', 'ней', 'ней',
  'оно', 'нему', 'ним', 'нас', 'нам', 'нами', 'вы', 'вас', 'вам', 'вами', 'ним', 'ними', 'ими'
];
const DIST_POSSESSIVE = ['своей', 'своих', 'мой', 'моего', 'моему', 'моим', 'моем', 'моя', 'моей', 'мою', 'моё',
  'мои', 'моих', 'моими', 'твой', 'твоего', 'твоему', 'твоим', 'твоем', 'твоя', 'твоей', 'твою', 'твоё',
  'твои', 'твоих', 'твоими', 'наш', 'нашего', 'нашему', 'нашим', 'нашем', 'наша', 'нашей', 'нашу',
  'наши', 'наших', 'нашими', 'ваш', 'вашего', 'вашему', 'вашим', 'вашем', 'ваша', 'вашей', 'вашу',
  'ваши', 'ваших', 'вашими', 'его', 'её', 'их', 'свой', 'своего', 'своему', 'своим', 'своем',
  'своя', 'свою', 'своё', 'свои', 'своими'
];
const DIST_BIASED = ['Реальная', 'скромные', 'реальные', 'обычно', 'масштабные', 'хорошо', 'значительное', 'реальных',
...
const DIST_GENERALIZATION = ['всю', 'все', 'всегда', 'никогда', 'никто', 'каждый', 'любой', 'абсолютно все',
  'везде', 'всюду', 'нигде', 'полностью', 'целиком', 'единственный', 'без исключений', 'сплошь и рядом',
  'всем известно', 'стопроцентно', 'повсеместно', 'тотально', 'вечно', 'постоянно', 'безусловно все'
];
...
// Долженствование и обязанность
  ...
  // Желание и намерение
  'хочет', 'хотят', 'желаю', 'желает', 'намерен', 'намерена', 'намерены', 'собирается', 'собираются',
  'планирует', 'планируют',
...
const DICT = {
  ...
  amplifier: DICT_AMPLIFIER, input: DICT_INPUT, stamp: DICT_STAMP, weak: DICT_WEAK, vague: DICT_VAGUE
};
